...
-- Creamos una primary key doble (de dos columnas, pueden repetirse una de ellas, pero no ambas a la vez)

alter table enfermo
add constraint pk_enfermo
primary key (inscripcion, nss);

insert into enfermo values (11996, 'Languia M.', 'Goya 20', '16/05/56', 'M', '280862482');
select * from enfermo;

------------------------- FOREIGN KEY --------------------------
-- Creamos una relación entre empleados y departamentos. El campo de relación es dept_no
alter table emp
add constraint fk_emp_dept
foreign key (dept_no)
references dept (dept_no);
-- Dará error si el campo a referenciar no es PRIMARY KEY o UNIQUE, previamente eliminamos esa restricción así que tenemos que volver a ponerla.
alter table dept 
add constraint pk_dept
primary key (dept_no);
-- Probamos ahora si funciona, insertamos un empleado en un departamento que no existe
insert into emp values (11, 'Nuevo', 'EMPLEADO', 7902, '11/11/1998', 1, 1, 50);
-- Da error, restricción de integridad violada, clave principal no encontrada
-- Si cambiamos el departamento que no existe por un null, sí lo acepta y lo guarda. Para ello habría que restringir también que no acepte nulos.
insert into emp values (11, 'Nuevo', 'EMPLEADO', 7902, '11/11/1998', 1, 1, null);
----------------------------------------------------------------------
-- Vamos a probar la eliminación en cascada y SET NULL en cascada
...
-- Ha eliminado a todos los empleados del departamento 10, por eso no hay que hacer un CASCADE en el DELETE
